...
enum HeaderNorm {
    ...
    static let foreignAliases: [String: String] = [
        ...
        //   set incl. workouts + journal, from a real export. "fc_max_bpm" is shared with French above
        //   (same folded form); a Swift dictionary literal fatal-errors on a duplicate key, so it is NOT
        //   repeated here — the existing French entry already covers it. —
        ...
        "pct_de_oxigenio_no_sangue": "blood_oxygen_pct",   // "% de oxigênio no sangue" → leading % → pct_…
