...
class WhoopCsvImporterTest {
    ...
    // --- Localized (Brazilian Portuguese) headers ---------------------------------------------

    /** Diacritic-folded Portuguese headers map onto the canonical English keys (parity with Swift). */
    @Test
    fun portugueseHeaderAliasesNormalize() {
        ...
        // Leading "%" in "% de oxigênio no sangue" becomes "pct" at the front, then folds.
        ...
        // "FC máx." is shared with the French alias and must still resolve.
        assertEquals("max_hr_bpm", HeaderNorm.normalize("FC máx. (bpm)"))
        assertEquals("average_hr_bpm", HeaderNorm.normalize("FC média (bpm)"))
    }

    /** A real ciclos_fisiológicos.csv header + one data row: values flow through the Portuguese aliases. */
    @Test
