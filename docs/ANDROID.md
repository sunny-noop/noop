...
- `data/Entities.kt` — Room `@Entity` classes mirroring the GRDB schema: `DeviceRow`, `HrSample`,
  ...
  `OnConflictStrategy.IGNORE`). Importers cover **WHOOP CSV**, **Apple Health**, **Health Connect**,
  and **raw `capture.json`** offloads (see [Raw capture import](#raw-capture-import-capturejson)).
...
**Pipeline** (`com.noop.ingest.CaptureImporter`, with `CaptureImporterTest` covering the pure parts):

1. **Parse — streamed.** A `capture.json` for a multi-day offload is tens of MB and hundreds of
   thousands of frames; loading it whole into a `String` + `JSONArray` OOMs a phone. Instead a small
   `JsonArrayScanner` pulls one top-level array element at a time off the SAF input stream (respecting
   strings, escapes and nested braces), handing each object to `org.json` for field parsing. Peak
   memory is the per-family frame lists plus one element substring — not the whole file.
2. **Decode — reuses the live path.** Frames are filtered to offload frames and run through the SAME
   ...
   records carry their own unix). There is no second decoder to drift.
...
5. **Recompute.** Raw samples are stored, but the dashboard reads *computed* recovery / strain / sleep,
   produced by `IntelligenceEngine` (persisted under `my-whoop-noop`, merged back by
   `daysMergedFlow`). That scoring pass otherwise only runs on the launch + 15-min analyze loop, so the
   UI wires `AppViewModel.analyzeImported(firstDay)` to fire one pass right after the import — widening
   the look-back (`analyzeWindowDays`) to cover the import's oldest day so an old historical capture is
   scored, not just the recent window. The dashboard then populates with no app restart.
