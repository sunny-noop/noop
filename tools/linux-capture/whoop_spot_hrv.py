#!/usr/bin/env python3
"""Compute spot HRV (RMSSD) on Linux from the strap's sparse PPG bursts in the offload.
...
14 bursts), so beats can be detected and RMSSD computed. Where a burst covers WHOOP's deep-sleep HRV
window, the PPG-derived RMSSD matches the cloud's value (validated: 111 ms vs cloud 110 on a captured
night, vs the offload RR field's 99).
